...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  int cardResult = -5; //used for return value of refactored cards
  ...
  switch( card ) 
    {
    case adventurer:
      cardResult =  adventurer_card(state, temphand);
      return cardResult;
/*
...
*/			
    case council_room:
      cardResult = councilRoom_card(state, currentPlayer, handPos, i);
      return cardResult;
/*
      //+4 Cards
      for (i = 0; i < 4; i++)
...
*/			
    ...
    case smithy:
      cardResult = smithy_card(i, state, currentPlayer, handPos);
      return cardResult;
/*
      //+3 Cards
      for (i = 0; i < 3; i++)
...
*/		
    case village:
      cardResult = village_card(state, currentPlayer, handPos);
      return cardResult;
/*
      //+1 Card
      drawCard(currentPlayer, state);
...
*/		
    ...
    case great_hall:
      cardResult = greatHall_card(state, currentPlayer, handPos);
      return cardResult;
/*
      //+1 Card
      drawCard(currentPlayer, state);
...
int adventurer_card(struct gameState *state, int *temphand)
{
   ...
   while(drawntreasure < 2)
   {
      if(state->deckCount[currentPlayer] < 1) //if the deck is empty, shuffle the discard pile to create new deck
      {
         shuffle(currentPlayer, state);
      }
      drawCard(currentPlayer, state);
      ...
      if(cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
      {
         drawntreasure++;  //BUG (FIXED), should be ++
      }
      else
      {
         temphand[z] = cardDrawn;
         state->handCount[currentPlayer]--; //should remove the top card (most recently drawn card)
         z++;
      }
   }
   while(z - 1 >= 0)
...
{
   //draw 4 cards
   for(i = 0; i < 4; i++)  //BUG (FIXED) - should be "i < 4"
   {
      drawCard(currentPlayer, state);
   }
...
{
   //draw 3 cards
   for(i = 0; i < 3; i++)  //BUG (FIXED) - should be "i < 3"
   {
      drawCard(currentPlayer, state);
   }
...
int village_card(struct gameState *state, int currentPlayer, int handPos)
{
   ...
   //gain two additional actions
   state->numActions = state->numActions + 2;  //BUG (FIXED) - should be state->numActions
