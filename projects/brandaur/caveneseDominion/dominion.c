...
int playAdventurer(struct gameState *state) {
  ...
  while(drawntreasure<2){
    ...
    drawCard(currentPlayer, state) //Emily's bug(FIXED): this is the line of code that was removed to introduce a bug.
...
int playCouncilRoom(struct gameState *state, int handPos) {
  ...
  //+1 Buy
  state->numBuys++;   //Emily's bug (FIXED): reduces number of buys by one instead of increases it
...
int playSmithy(struct gameState *state, int handPos) {
  ...
  //+3 Cards
  for (i = 0; i < 3; i++) //Emily's bug (FIXED): she added an =, which should result in 4 cards being drawn
  {
    drawCard(currentPlayer, state);
  }
...
//Did not seem to find a bug for this card...
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case steward:
      playSteward(choice1, choice2, choice3, state, handPos);
      return 0;
