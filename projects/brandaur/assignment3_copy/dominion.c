...
{
   //draw 4 cards
   for(i = 0; i < 2; i++)  //BUG - should be "i < 4"
   {
      drawCard(currentPlayer, state);
   }
...
{
   //draw 3 cards
   for(i = 0; i > 3; i++)  //BUG - should be "i < 3"
   {
      drawCard(currentPlayer, state);
   }
...
int village_card(struct gameState *state, int currentPlayer, int handPos)
{
   ...
   state->numBuys = state->numActions + 2;  //BUG - should be state->numActions
